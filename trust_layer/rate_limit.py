...
Hot path (normal proofs within quota): Redis INCR — atomique, sans lock.
Cold path (overage, alertes): JSON avec _RATE_LIMIT_LOCK — conservé tel quel.
Fallback automatique vers JSON si Redis est indisponible.
"""
    ...
    with _RATE_LIMIT_LOCK:
        ...
        if monthly_limit is not None:
            ...
            if monthly_remaining > 0:
                ...
                remaining = min(max(0, limit - entry["count"]), monthly_remaining - 1)
                limits[key_id] = entry
