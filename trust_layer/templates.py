...
def render_proof_page(proof: dict, integrity_verified: bool) -> str:
    ...
    # The chain hash algorithm is selected by spec_version (proof-spec section 2).
    if spec_version in ("3.0", "3.1"):
        algorithm_desc = "RFC 6962 Merkle root of per-field commitments (proof-spec section 2)"
    else:
        algorithm_desc = ("SHA-256(request + response + payment_id + timestamp + buyer + seller"
                          + (" + upstream_timestamp" if upstream_timestamp else "")
                          + (" + receipt_hash" if provider_payment.get("receipt_content_hash") else "")
                          + ")")
    ...
    if not integrity_verified:
        ...
        verdict_icon = "\u2705"
        verdict_text = "PROOF VERIFIED"
        verdict_sub = ("Chain hash and ArkForge signature check out, and the batch is anchored with an "
                       "RFC 3161 timestamp and a Sigstore Rekor entry. This page shows what went through "
                       "the proxy, not whether the response was correct.")
    else:
        verdict_bg = "#052e16"
        verdict_border = "#22c55e"
        verdict_icon = "\u2705"
        verdict_text = "CHAIN HASH VERIFIED"
        verdict_sub = ("The chain hash checks out. The external anchors are not confirmed on this page: "
                       "check the RFC 3161 timestamp and the Sigstore Rekor entry with the verifier.")
...
{provider_payment_html}
    ...
    <details>
        ...
        <div class="tech-inner">
            ...
            <div class="tech-row"><span class="tech-label">Algorithm</span><span class="tech-val">{algorithm_desc}</span></div>
        </div>
    </details>

    <!-- 8. CTA BANNER -->
    <div class="cta-banner">
        <p>This proof was generated by <a href="https://arkforge.tech/en/pricing?utm_source=proof_view&utm_medium=proof_page&utm_content=cta_banner">ArkForge Trust Layer</a>. Give every AI agent call a proof a third party can check: <a href="https://arkforge.tech/en/pricing?utm_source=proof_view&utm_medium=proof_page&utm_content=cta_banner">get started free</a>.</p>
    </div>

    <!-- 9. FOOTER -->
    <div class="footer">
        <div class="tagline">ArkForge Trust Layer — non-repudiation for autonomous AI systems.</div>
        <div class="arkforge"><a href="https://arkforge.tech" style="display:inline-flex;align-items:center;gap:6px"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="20" height="20" style="vertical-align:middle"><rect width="64" height="64" rx="14" fill="#0B0F14"/><line x1="12" y1="32" x2="52" y2="32" stroke="#00E5FF" stroke-width="5" stroke-linecap="round"/><circle cx="32" cy="32" r="14" stroke="#FFF" stroke-width="4" fill="none"/><circle cx="32" cy="32" r="5" fill="#00E5FF"/></svg> arkforge.tech</a></div>
    </div>
